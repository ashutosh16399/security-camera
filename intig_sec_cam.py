# -*- coding: utf-8 -*-
"""
Created on Sat Jul  4 01:26:44 2020
...
#x = ''.join(random.choices(string.ascii_letters , k=5))
#x = x + '.avi'
#out = cv2.VideoWriter(x, cv2.VideoWriter_fourcc(*'XVID'), 25, (640, 480))
...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if time.time() >= future1-1:
            ...
            print(x)
            print(name)
            cv2.imwrite(x+'.jpg', frame[top-25:bottom+25,left-25:right+25])
            stor.child(x+".jpg").put(x+".jpg")
            data={"id":x,"time":str(datetime.now()),"name":name}
    ...
    if v_f and (name == "Unknown") and (time.time() >= future-1) :
        v_f = 0
        x = ''.join(random.choices(string.ascii_letters , k=5))
    ...
    if flag and (time.time() > future-1):
        flag = 0
        #out.release()
